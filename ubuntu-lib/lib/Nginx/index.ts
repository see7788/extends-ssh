...
class Nginx extends Base<(input: number | string) => Promise<Current>> {
  ...
  async getRemote(port: number): Promise<Current> {
    ...
    return {
      subdomain: this.remoteDescription(value),
  ...
  async makeRemote(input: number | string): Promise<Current> {
    ...
    if (typeof input === "number") {
      const port = inputValidator.parse({ port: input }).port;
    ...
    const path = pathValidator.parse(input);
    const basename = posix.basename(path);
    const port = Number(basename);
    if (String(port) !== basename) throw new Error(`远程路径末段必须是开发端口: ${path}`);
    const validPort = devPortValidator.parse(port);
    await this.writeRemote(validPort, `  location / { root ${path}; index index.html; try_files $uri $uri/ =404; }`);
    return {
      subdomain: this.remoteDescription(validPort),
      hasRemote: () => this.hasRemote(validPort),
      close: () => this.closeRemote(validPort),
    };
  }
